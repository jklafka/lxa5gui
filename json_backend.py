...
class graphics_box(object):
	...
	"""
	def __init__(self, data_box, svg, upper_left, box_size, zoom):
		...
		if self.data_box.inner_boxes != []:
			...
			self.add_text(self._svg, words_list, \
				...
				upper_left[1] + box_size[1] / 4), box_size[1], zoom)
	...
	def add_text(self, svg, text, location, box_len, zoom):
		'''
		...
		for word in text[1:]:
			if len(textstr) > 50:
				...
				textstr += ", "
				textstr += word
		svg.add(svg.text(textstr, insert = location, style = "font-size:" + \
			str(int(16 * zoom)) + "px"))
		if textstr2 != "":
			svg.add(svg.text(textstr2, insert = (location[0], location[1] + \
				BASE_SIZE * 2), style = "font-size:" + \
				str(int(16 * zoom)) + "px"))
...
def main(signatures_filename, sort_crit=None, filter_crit=None, zoom=1):
	...
	if sort_crit == "robustness":
		outer_box.inner_boxes.sort(key=lambda x : x[0].text[2], reverse = True)
	elif sort_crit == "stems":
		outer_box.inner_boxes.sort(key=lambda x : x[0].text[0], reverse = True)
	elif sort_crit == "suffixes":
		outer_box.inner_boxes.sort(key=lambda x : x[0].text[1], reverse = True)
	...
	for box in outer_box.inner_boxes:
		graphics_box(box[0], svg_document, \
			...
			(800 * zoom, zoom * (box_param - BASE_SIZE)), zoom)
	svg_document.save()
